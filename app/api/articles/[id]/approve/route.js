...
export async function POST(request, { params }) {
  ...
  // Only editors and superusers can approve articles
  ...
  try {
    ...
    // Article must be in review status to be approved
    if (article.workflow_status !== 'in_review') {
      return Response.json({ 
        error: 'Only articles in review can be approved' 
      }, { status: 400 });
    }
    ...
    return Response.json({ error: 'Failed to approve article' }, { status: 500 });
  }
}
